...
export const handleOnZEVMWithdrawn = async ({
  ...
  try {
    const isZETA = args[1] !== 0;
    const receiver = args[2];
    const zrc20 = args[3];
    const amount = args[4];
    const message = args[7];
    if (isZETA) {
      return await protocolContracts.zetaConnector
        .connect(tss)
        .withdraw(
          receiver,
          amount,
      ...
      if (message !== "0x") {
        ...
        if (isGasToken) {
          ...
          console.log("!!!");
          const tx = await protocolContracts.connector
            .connect(tss)
            .withdraw(receiver, amount, "", deployOpts);
        } else {
          const erc20 = getERC20ByZRC20(zrc20);
