...
async function withdraw(args: any): Promise<void> {
    ...
    console.log(`Action: withdraw ${coinsPretty(amount)} to ${addrPretty(recipient)}`);
    ...
    const approveTx = await tonToken.approve(args.gateway, parseAmount(10_000_000))
...
async function callWithdrawal(
    ...
    const method = "withdraw(bytes,uint256,address,(address,bool,address,bytes,uint256))"
