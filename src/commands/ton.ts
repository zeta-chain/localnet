...
import { GatewayZEVMContract, ZRC20Contract } from "../types/contracts";
import { retry, tonFormatCoin } from "../utils";

const showBalance = async (args: { address: string }): Promise<void> => {
  const c = client();
...
const createWallet = async (args: { amount: string }): Promise<void> => {
  const c = client();
  const deployer = await deployerFromFaucetURL(ENDPOINT_FAUCET, c);
...
const withdraw = async (args: {
  ...
  const tonToken = new ethers.Contract(
    args.token,
    ZRC20.abi,
    wallet
  ) as ZRC20Contract;
  ...
  const tonBalance = (await tonToken.balanceOf(wallet.address)) as bigint;
...
const coinsPretty = (balance: bigint): string => {
  return `${pc.yellow(tonFormatCoin(balance))} ${pc.blue("▼ ton")}`;
};

const addrPretty = (addr: Address): string => {
